...
** Description:             change intensity TEST
** alpha represents the intensity factor (0: dark, 100: max intensity)
** added by Peter Jonas (DL5DLA)
*************************************************************************************x*/
uint32_t TFT_eSPI::intensity(uint8_t alpha, uint32_t color)
{
  // For speed use fixed point maths and rounding to permit a power of 2 division
  uint16_t colorR = (color >> 11) &  0b11111;
  uint16_t colorG = (color >>  5) & 0b111111;
  uint16_t colorB = (color >>  0) &  0b11111;

  // Shift right 1 to drop rounding bit and shift right 8 to divide by 256
  uint16_t r = (colorR * alpha/100);
  if(r>0b11111) r=0b11111;
  uint16_t g = (colorG * alpha/100);
  if(g>0b111111) g=0b111111;
  uint16_t b = (colorB * alpha/100);
  if(b>0b11111) b=0b11111;

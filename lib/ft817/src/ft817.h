...
class FT817
{
	public:
		...
		byte PTT(boolean toggle);		// ptt/un-ptt
		...
		bool toggleIPO();				// toggle the IPO status for the current VFO, switching
										// breifly to the other VFO and back, returns true is success
		bool toggleATT();				// toggle the ATT status for the current VFO, switching
										// breifly to the other VFO and back, returns true is success
		bool toggleNB();				// toggle the NB (noise blanker)
		...
		bool setAnt(byte bands);	 // set new rear/front antenna config
		bool setBandVFO(bool vfo, byte band);
		bool setAGC(byte);
		bool setChargeTime(byte);
		bool setPWR(byte);
		bool setSteps(byte);
		bool enableCharging(bool);
		...
		bool getATT();					// get the ATT status for the actual VFO
		bool getNB();					// get the NB status for the noise blanker
		byte getSteps();				// get the state of steps for the actual VFO
		bool getBreakIn();				// get the Break In operation status
		bool getKeyer();				// toggle Keyer
		byte getAnt();					// get current rear/front antenna config
		bool getLock();
		byte getPowerSets();			// get the current configured TX power
		byte getAGC();					// get current AGC settings
		byte getChargeTime();			// get charging time
		uint16_t read_TX_Meter();		// read TX metering
		byte get_Byte57();
		bool get_CHG_Status();			// curently charging or not
	...
	private:
		...
		bool writeEEPROM(byte data);	// write data (performs a read cycle inside to preserve nextByte)
										...
										// the wrote data.
		byte getByteFromEEPROM();		// get a byte position from an eeprom address loaded in MSB/LSB
		...
		bool getBitFromVFO(signed int offset, byte rbit);	// this is a nice trick, it will return the bit
															...
															// offset in bytes...
		byte getByteFromVFO(signed int offset);
